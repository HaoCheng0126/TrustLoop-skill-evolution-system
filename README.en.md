...
Install the full TrustLoop experience with the plugin package:
...
- the plugin brings the bundled TrustLoop skill with it
- restart Gateway or restart OpenClaw so plugin changes are loaded
- start a new session and TrustLoop is ready to work

### Standalone fallback: skill-only

If you want to try the workflow without the native managed-skill tool, install just the skill:

```bash
openclaw skills install trustloop-skill-evolver
```

This path is useful for quick evaluation, but the plugin-backed install is the recommended production setup.
