...
7. Read the current autonomy mode from `./.skill-evolver/config.json` or from the native tool.
8. Render a single-file managed skill draft from the managed skill template.
9. Write a candidate record to `./.skill-evolver/candidates/<candidate-id>.md`.
10. Insert or update the registry record in `./.skill-evolver/registry.json`.
11. Write a structured audit event into `./.skill-evolver/audit/`.
12. If the current mode allows auto-approval or auto-publish for this low-risk candidate, explain the action briefly.
13. Otherwise reply with a short approval request such as: `I found a reusable workflow and created candidate <id> for learned-foo. Publish it?`
